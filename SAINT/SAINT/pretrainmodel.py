...
class SAINT(model.nn.Module):
    def __init__(
        ...
        categories_offset = model.F.pad(model.model.torch.tensor(list(categories)), (1, 0), value=num_special_tokens)
        ...
        if self.cont_embeddings == 'MLP':
            self.simple_MLP = model.nn.ModuleList([model.simple_MLP([1, 100, self.dim]) for _ in range(self.num_continuous)])
        ...
        cat_mask_offset = model.F.pad(model.model.torch.Tensor(self.num_categories).fill_(2).type(model.model.torch.int8), (1, 0), value=0)
        cat_mask_offset = cat_mask_offset.cumsum(dim=-1)[:-1]

        con_mask_offset = model.F.pad(model.model.torch.Tensor(self.num_continuous).fill_(2).type(model.model.torch.int8), (1, 0), value=0)
